...
class WakeUp {
  ...
  async isTaskCreated() {
    try {
      ...
      if (err?.message?.search('cannot find the file') !== -1) return Promise.resolve(false);
  ...
  async createTask(time) {
    try {
      ...
      const {stdout} = await execAsync('schtasks /Create /XML task.xml /Tn MSMZ');
      console.log(stdout);
  ...
  async createConfig(time) {
    try {
      ...
      if (userInfo) {
        ...
        let patternTask = await fs.promises.readFile(getPublicPath() + 'task.txt', 'utf-8');
        ...
        patternTask = patternTask
          ...
          .replace(
            '$DATE_TRIGGER',
            dayjs().set('hour', timeArr[0]).set('minute', timeArr[1]).set('millisecond', 0).subtract(5, 'minute').format('YYYY-MM-DDTHH:mm:ss')
          )
  ...
  async #getUserInfo() {
    try {
      ...
      const {stdout} = await execAsync(`wmic useraccount where name='${os.userInfo().username}' get sid`);
...
class Vpn {
  ...
  async #isConnected() {
    try {
      const {
        stdout,
      } = await execAsync('trac info', {cwd: this.pathVpn, signal: this.stopConnect.signal});

      if (stdout?.search("status: Connecting") !== -1) {
        await execAsync('trac disconnect', {cwd: this.pathVpn, signal: this.stopConnect.signal})
      ...
      this.#addLog(`${dayjs().format('DD.MMMM.YYYY HH:mm')} [statusVpn] - ${err.message}`, true);
  ...
  async connect(config) {
    try {
      ...
      if (stdout) {
        const isConnect = stdout.search("Connection was successfully") !== -1 || stdout.search("Client is already connected") !== -1;

        if (!isConnect) {
          if (stdout.search("previous connection is currently on progress") !== -1) {
            return await this.connect(config);
          }

          this.#addLog(`${dayjs().format('DD.MMMM.YYYY HH:mm')} [connectVpn] - Не смог подключиться к впн ${stdout}`, true);
        }
      ...
      if (err.stderr) {
        ...
        this.#addLog(`${dayjs().format('DD.MMMM.YYYY HH:mm')} [connectVpn] - ${err.stderr}`, true);
      }
...
class Messager {
  ...
  #startTimer(time, fn, isEnd) {
    const timeArr = time.split(':');
    let targetDate = dayjs().set('hour', timeArr[0]).set('minute', timeArr[1]).set('millisecond', 0);
  ...
  async #sendMessage(title, text) {
    if (!await LockSimultaneousCall.exec(() => this.vpnService.connect(this.config))) {
      return Promise.reject(false);
    }

    const service = new ews.ExchangeService(ews.ExchangeVersion.Exchange2013);
    service.Credentials = new ews.WebCredentials(this.config.email, this.config.password);
    ...
    return emailMessage.SendAndSaveCopy()
      .then(() => {
  ...
  #valid(config) {
    if (config.vpnUser.search(/\\/gu) !== -1 || config.vpnUser.search(/\\/gu) !== -1) {
      this.#addLog(`${dayjs().format('DD.MMMM.YYYY HH:mm')} [connectVpn] - не безопасные символы \\`, true);
      return false
    }
...
class TabsViewContent {
  ...
  createTabs() {
    ...
    view.setBounds({x: 0, y: 0, width: this.win.getBounds().width, height: this.heightTabs});
  ...
  closeViewContent(id) {
    const view = this.win.contentView.children.find(v => v.webContents.id === id);
